...
// sourceId maps to backend DataSource id strings
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "0.6rem" }}>
              {["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA", "NVDA", "META", "SPY"].map((t) => (
                <button
                  key={t}
                  type="button"
                  style={chipStyle(fundamentalTicker === t)}
                  onClick={() => setFundamentalTicker(t)}
                >
                  {t}
                </button>
              ))}
              ...
              {[
                ...
                return (
                  <button
                    ...
                    onClick={() => { setProjectionFrom(from); setProjectionTo(to); }}
                  >
                    {label}
                  </button>
                );
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
              {INVESTMENT_PROFILES.map((p) => (
                <button
                  key={p}
                  type="button"
                  style={chipStyle(investmentProfile === p)}
                  onClick={() => setInvestmentProfile(p)}
                >
                  {p}
                </button>
              ))}
              ...
              {FUNDAMENTAL_METRICS.map((m) => (
                ...
                >
                  <input
                    ...
                    style={{ accentColor: "var(--color-accent, #ffd43b)", width: 15, height: 15 }}
                  />
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "0.6rem" }}>
              ...
              <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem", fontSize: "0.75rem" }}>
                ...
                <input
                  ...
                  placeholder="ej. 0.5"
                  value={debtEquityMax}
                  onChange={(e) => setDebtEquityMax(e.target.value)}
                />
              </label>
              <label style={{ display: "flex", flexDirection: "column", gap: "0.2rem", fontSize: "0.75rem" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
              {OPTIONS_STRATEGIES.map((s) => (
                <button
                  key={s}
                  type="button"
                  style={chipStyle(optionsStrategy === s)}
                  onClick={() => setOptionsStrategy(s)}
                >
                  {s}
                </button>
              ))}
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
              {COMPARISON_OPTIONS.map((c) => (
                <label
                  key={c}
                  style={{ display: "flex", alignItems: "center", gap: "0.6rem", cursor: "pointer", fontSize: "0.82rem" }}
