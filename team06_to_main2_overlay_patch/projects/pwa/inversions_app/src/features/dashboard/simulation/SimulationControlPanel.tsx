...
export function SimulationControlPanel({
  ...
  const { incrementSimulationRunCount, selectedStrike } = useSignalStore();
  ...
  // FIC: Auto-fill Bear/Bull spread fields from the option-chain row selected by the user. (EN)
  // FIC: Autorrellenar campos Bear/Bull con la fila elegida en Cadena de Opciones. (ES)
  useEffect(() => {
    if (!spreadModalOpen || !isSpreadStrategy(estrategia) || !selectedStrike) return;

    const defaults = selectedStrike.strategyDefaults?.[estrategia as keyof NonNullable<typeof selectedStrike.strategyDefaults>];
    const currentPrice = selectedStrike.underlyingPrice ?? spreadParams.currentPrice;

    if (defaults) {
      setSpreadParams((prev) => ({
        ...prev,
        currentPrice: currentPrice > 0 ? currentPrice : prev.currentPrice,
        longStrike: defaults.longStrike,
        longPremium: Number(defaults.longPremium.toFixed(2)),
        shortStrike: defaults.shortStrike,
        shortPremium: Number(defaults.shortPremium.toFixed(2)),
        contracts: prev.contracts || 1,
      }));
      if (selectedStrike.expiration) setEstrategiaTo(selectedStrike.expiration);
      return;
    }
    ...
    setSpreadParams((prev) => {
      ...
      if (selectedStrike.type === "call") {
        if (estrategia === "BULL_CALL_SPREAD") {
          next.longStrike = selectedStrike.strike;
          next.longPremium = Number(selectedStrike.premium.toFixed(2));
        } else if (estrategia === "BEAR_CALL_SPREAD") {
          next.shortStrike = selectedStrike.strike;
          next.shortPremium = Number(selectedStrike.premium.toFixed(2));
        }
      }

      if (selectedStrike.type === "put") {
        if (estrategia === "BEAR_PUT_SPREAD") {
          next.longStrike = selectedStrike.strike;
          next.longPremium = Number(selectedStrike.premium.toFixed(2));
        } else if (estrategia === "BULL_PUT_SPREAD") {
          next.shortStrike = selectedStrike.strike;
          next.shortPremium = Number(selectedStrike.premium.toFixed(2));
        }
      }
    ...
    if (selectedStrike.expiration) setEstrategiaTo(selectedStrike.expiration);
  }, [estrategia, selectedStrike, spreadModalOpen, spreadParams.currentPrice]);
  useEffect(() => {
